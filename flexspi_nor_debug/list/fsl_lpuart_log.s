///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V8.32.3.193/W32 for ARM        11/Jun/2019  09:38:26
// Copyright 1999-2019 IAR Systems AB.
//
//    Cpu mode     =  
//    Endian       =  little
//    Source file  =  
//        C:\Development\smart_washing_machine_3080\source\fsl_lpuart_log.c
//    Command line =  
//        -f C:\Users\nxf42695\AppData\Local\Temp\1\EW75C8.tmp
//        (C:\Development\smart_washing_machine_3080\source\fsl_lpuart_log.c -D
...
//    Locale       =  C
//    List file    =  
//        C:\Development\smart_washing_machine_3080\flexspi_nor_debug\list\fsl_lpuart_log.s
//
///////////////////////////////////////////////////////////////////////////////
        ...
        PUBLIC LOG_LPUART_RTOS_Deinit
        PUBLIC LOG_LPUART_RTOS_Init
        PUBLIC LOG_LPUART_RTOS_Receive
        PUBLIC LOG_LPUART_RTOS_Send
...
//   18 
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function LOG_LPUART_RTOS_Callback
        THUMB
//   19 static void LOG_LPUART_RTOS_Callback(LPUART_Type *base, lpuart_handle_t *state, status_t status, void *param)
//   20 {
LOG_LPUART_RTOS_Callback:
        PUSH     {R3-R5,LR}
          CFI R14 Frame(CFA, -4)
          CFI R5 Frame(CFA, -8)
          CFI R4 Frame(CFA, -12)
...
//   27     if (status == kStatus_LPUART_RxIdle)
        MOVW     R1,#+1303
        CMP      R2,R1
        BNE.N    ??LOG_LPUART_RTOS_Callback_0
//   28     {
//   29         xResult = xEventGroupSetBitsFromISR(handle->rxEvent, RTOS_LPUART_COMPLETE, &xHigherPriorityTaskWoken);
        ...
        B.N      ??LOG_LPUART_RTOS_Callback_1
//   30     }
//   31     else if (status == kStatus_LPUART_TxIdle)
??LOG_LPUART_RTOS_Callback_0:
        MOVW     R1,#+1302
        CMP      R2,R1
        BNE.N    ??LOG_LPUART_RTOS_Callback_2
//   32     {
...
??LOG_LPUART_RTOS_Callback_2:
        MOVW     R1,#+1308
        CMP      R2,R1
        BNE.N    ??LOG_LPUART_RTOS_Callback_3
//   36     {
//   37         xResult =
...
??LOG_LPUART_RTOS_Callback_3:
        MOVW     R1,#+1309
        CMP      R2,R1
        BNE.N    ??LOG_LPUART_RTOS_Callback_1
//   41     {
...
??LOG_LPUART_RTOS_Callback_1:
        CMP      R0,#+0
        BEQ.N    ??LOG_LPUART_RTOS_Callback_4
//   49     {
...
//   67  */
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function LOG_LPUART_RTOS_Init
        THUMB
//   68 int LOG_LPUART_RTOS_Init(lpuart_rtos_handle_t *handle, lpuart_handle_t *t_handle, const lpuart_rtos_config_t *cfg)
//   69 {
LOG_LPUART_RTOS_Init:
        PUSH     {R4,R5,LR}
          CFI R14 Frame(CFA, -4)
          CFI R5 Frame(CFA, -8)
          CFI R4 Frame(CFA, -12)
...
//   92     if (0 == cfg->baudrate)
        LDR      R0,[R5, #+8]
        CMP      R0,#+0
        BNE.N    ??LOG_LPUART_RTOS_Init_1
//   93     {
//   94         return kStatus_InvalidArgument;
??LOG_LPUART_RTOS_Init_0:
        MOVS     R0,#+4
        B.N      ??LOG_LPUART_RTOS_Init_2
//   95     }
//   96 
//   97     handle->base    = cfg->base;
??LOG_LPUART_RTOS_Init_1:
        LDR      R0,[R5, #+0]
        STR      R0,[R4, #+0]
...
//  147     LPUART_Init(handle->base, &defcfg, cfg->srcclk);
        ...
        BL       LPUART_Init
//  148     LPUART_TransferCreateHandle(handle->base, handle->t_state, LOG_LPUART_RTOS_Callback, handle);
        MOV      R3,R4
...
//  170  */
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function LOG_LPUART_RTOS_Deinit
        THUMB
//  171 int LOG_LPUART_RTOS_Deinit(lpuart_rtos_handle_t *handle)
//  172 {
LOG_LPUART_RTOS_Deinit:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
...
//  207  */
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function LOG_LPUART_RTOS_Send
        THUMB
//  208 int LOG_LPUART_RTOS_Send(lpuart_rtos_handle_t *handle, const uint8_t *buffer, uint32_t length, uint32_t timeout)
//  209 {
LOG_LPUART_RTOS_Send:
        PUSH     {R2-R8,LR}
          CFI R14 Frame(CFA, -4)
          CFI R8 Frame(CFA, -8)
          CFI R7 Frame(CFA, -12)
...
//  218     if (0 == length)
        CMP      R4,#+0
        BNE.N    ??LOG_LPUART_RTOS_Send_1
//  219     {
//  220         return 0;
        MOV      R0,R8
        B.N      ??LOG_LPUART_RTOS_Send_2
//  221     }
//  222     if (NULL == buffer)
??LOG_LPUART_RTOS_Send_1:
        CMP      R5,#+0
        BNE.N    ??LOG_LPUART_RTOS_Send_3
//  223     {
...
//  250 
//  251     return retval;
??LOG_LPUART_RTOS_Send_6:
        MOV      R0,R8
??LOG_LPUART_RTOS_Send_2:
...
//  270  */
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function LOG_LPUART_RTOS_Receive
        THUMB
//  271 int LOG_LPUART_RTOS_Receive(lpuart_rtos_handle_t *handle, uint8_t *buffer, uint32_t length, size_t *received, uint32_t timeout)
//  272 {
LOG_LPUART_RTOS_Receive:
        PUSH     {R2-R10,LR}
          CFI R14 Frame(CFA, -4)
          CFI R10 Frame(CFA, -8)
          CFI R9 Frame(CFA, -12)
...
//  278     if (NULL == handle->base)
        LDR      R0,[R7, #+0]
        CMP      R0,#+0
        BEQ.N    ??LOG_LPUART_RTOS_Receive_0
//  279     {
//  280         /* Invalid handle. */
...
//  283     if (0 == length)
        CMP      R5,#+0
        BNE.N    ??LOG_LPUART_RTOS_Receive_1
//  284     {
//  285         if (received != NULL)
        CMP      R4,#+0
        BEQ.N    ??LOG_LPUART_RTOS_Receive_2
//  286         {
//  287             *received = n;
        MOV      R0,R8
        STR      R0,[R4, #+0]
//  288         }
//  289         return 0;
??LOG_LPUART_RTOS_Receive_2:
        MOV      R0,R8
        B.N      ??LOG_LPUART_RTOS_Receive_3
//  290     }
//  291     if (NULL == buffer)
??LOG_LPUART_RTOS_Receive_1:
        CMP      R6,#+0
        BNE.N    ??LOG_LPUART_RTOS_Receive_4
//  292     {
...
??LOG_LPUART_RTOS_Receive_4:
        MOV      R1,#-1
        LDR      R0,[R7, #+20]
          CFI FunCall xQueueSemaphoreTake
        BL       xQueueSemaphoreTake
        CMP      R0,#+0
        BNE.N    ??LOG_LPUART_RTOS_Receive_5
//  298     {
//  299         /* We could not take the semaphore, exit with 0 data received */
//  300         return kStatus_Fail;
??LOG_LPUART_RTOS_Receive_0:
        MOVS     R0,#+1
        B.N      ??LOG_LPUART_RTOS_Receive_3
//  301     }
??LOG_LPUART_RTOS_Receive_5:
        LDR      R10,[SP, #+40]
//  302 
...
//  312     if (ev & RTOS_LPUART_HARDWARE_BUFFER_OVERRUN)
        LSLS     R1,R0,#+29
        BPL.N    ??LOG_LPUART_RTOS_Receive_6
//  313     {
...
//  320         local_received = 0;
        B.N      ??LOG_LPUART_RTOS_Receive_7
//  321     }
//  322     else if (ev & RTOS_LPUART_RING_BUFFER_OVERRUN)
??LOG_LPUART_RTOS_Receive_6:
        LSLS     R1,R0,#+30
        BPL.N    ??LOG_LPUART_RTOS_Receive_8
//  323     {
...
??LOG_LPUART_RTOS_Receive_8:
        LSLS     R0,R0,#+31
        BPL.N    ??LOG_LPUART_RTOS_Receive_7
//  333     {
...
??LOG_LPUART_RTOS_Receive_7:
        CMP      R4,#+0
        BEQ.N    ??LOG_LPUART_RTOS_Receive_9
//  340     {
...
??LOG_LPUART_RTOS_Receive_9:
        ...
        BL       xQueueGenericSend
        CMP      R0,#+0
        BNE.N    ??LOG_LPUART_RTOS_Receive_10
//  346     {
...
//  349     }
//  350     return retval;
??LOG_LPUART_RTOS_Receive_10:
        MOV      R0,R8
??LOG_LPUART_RTOS_Receive_3:
